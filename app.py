...
st.set_page_config(page_title="Moto Scanner V4.2", page_icon="🏍️", layout="wide")
st.markdown("""
<style>
...
def _slugify_lbc(text):
    s = str(text).lower().strip()
    s = re.sub(r"[^a-z0-9]+", "-", s)
    return re.sub(r"-+", "-", s).strip("-")

def _lbc_public_category_urls(model):
    """
    Public, indexable Leboncoin category/search pages.
    Useful on cloud hosts where /recherche or the unofficial API may return 0.
    """
    urls = []
    for alias in aliases_for(model):
        slug = _slugify_lbc(alias)
        if slug:
            urls.append(f"https://www.leboncoin.fr/ck/motos/{slug}")

    # Add compact/model-only slugs for common naming differences.
    brand = model.split()[0].lower() if model.split() else ""
    brandless = " ".join(model.split()[1:]) if len(model.split()) > 1 else model
    for candidate in [
        _slugify_lbc(brandless),
        _slugify_lbc(brandless.replace("-", "")),
        _slugify_lbc(model.replace("-", "")),
    ]:
        if candidate:
            urls.append(f"https://www.leboncoin.fr/ck/motos/{candidate}")

    # Useful broad Yamaha MT fallback, still filtered locally by exact model aliases.
    if normalize(model).startswith("yamahamt"):
        urls.append("https://www.leboncoin.fr/c/motos/u_moto_brand%3AYAMAHA%2Bu_moto_model%3AYAMAHA_MT")
...
def _parse_lbc_public_page(url, model, selected_radius):
    ads = []
    seen = set()
    ...
    for a in soup.find_all("a", href=True):
        ...
        for _ in range(9):
            ...
            node = node.parent
            txt = compact_text(node)
            if "€" in txt and "km" in txt.lower() and len(txt) < 5000:
                break
    ...
    # Some cloud-rendered category pages expose ad data in scripts rather than
    # normal card anchors. Extract direct /ad/ URLs and inspect nearby text.
    if not ads:
        raw = r.text
        for m in re.finditer(r'https?://(?:www\.)?leboncoin\.fr/ad/[^"\'<>\s\\\\]+', raw):
            full = m.group(0).replace("\\u002F","/").replace("\\/","/")
            if full in seen:
                continue

            start = max(0, m.start()-1800)
            end = min(len(raw), m.end()+1800)
            snippet = BeautifulSoup(raw[start:end], "html.parser").get_text(" ", strip=True)

            if not text_matches_model(snippet, model):
                continue
            ...
            cp = postcode_from_text(snippet)
            ad = {
                "source":"Leboncoin","model":model,"year":int(year),"km":int(km),
                "price":float(price),"location":cp or "","seller_type":"",
                "description":snippet[:1600],"options":extract_options(snippet),"url":full
            }
            ad["distance_km"] = distance_from_search_postcode(ad)
            if ad["distance_km"] is not None and ad["distance_km"] > selected_radius:
                continue
...
def leboncoin_search(model):
    ...
    ads = unique_by_url(ads)

    # Cloud fallback: public, indexable Leboncoin category pages.
    # This is deliberately after the V3.5.2 structured + HTML paths, so the PC
    # behaviour remains unchanged when those already work.
    if not ads:
        public_ads = []
        for public_url in _lbc_public_category_urls(model):
            time.sleep(0.6)
            public_ads.extend(_parse_lbc_public_page(public_url, model, selected_radius))
        ads = unique_by_url(public_ads)
...
st.sidebar.title("🏍️ Moto Scanner V4.2")
...
# UI
# -------------------------------------------------------------------
st.title("🏍️ Moto Scanner France — V4.2 Mobile")
...
if page == "Scanner Live":
    ...
    st.caption("V4.2 ajoute un fallback spécial hébergement cloud via les pages publiques indexables Leboncoin.")
    st.info(
...
""")
    ...
    st.write("Teste chaque source indépendamment. Leboncoin essaie maintenant : client structuré → /recherche → pages publiques /ck et /c, puis déduplique les URLs.")
